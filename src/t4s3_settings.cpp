...
uint32_t sanitize_maintenance_interval(uint32_t seconds, uint32_t fallback)
{
    if (seconds < MIN_MAINTENANCE_INTERVAL_SEC || seconds > MAX_MAINTENANCE_INTERVAL_SEC) {
        return fallback;
    }
    return seconds;
}
...
void t4s3_settings_load(T4S3UiSettings &settings)
{
    ...
    settings.maintenanceMachineIntervalSec = sanitize_maintenance_interval(
        prefs.getULong("maintMachInt", settings.maintenanceMachineIntervalSec),
        DEFAULT_MAINTENANCE_MACHINE_INTERVAL_SEC);
    settings.maintenanceGrinderIntervalSec = sanitize_maintenance_interval(
        prefs.getULong("maintGrindInt", settings.maintenanceGrinderIntervalSec),
        DEFAULT_MAINTENANCE_GRINDER_INTERVAL_SEC);
